...
class AcaoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        try:
            ...
            # Indicadores
            indicadores = [i.strip().replace(" ", "_").replace(f"DIVIDEND_YIELD__-_{response.meta['papel']}", "Dividend_Yield") for i in response.css('div.cell span.d-flex::text').getall() if i.strip()]
            ...
            # Informações da ação
            info = [i.strip().replace(" ", "_") for i in response.css('div.cell span.title::text').getall() if i.strip()]
